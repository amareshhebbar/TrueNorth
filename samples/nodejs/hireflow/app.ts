/**
 * HireFlow (Node.js / TypeScript) — AI Candidate Screening
 *
 * FILE STRUCTURE
 *   hireflow/
 *   ├── app.ts       ← this file
 *   ├── goal.yaml    ← copy from python/hireflow/goal.yaml
 *   ├── package.json
 *   └── tsconfig.json
 *
 * package.json:
 *   { "scripts": { "dev": "ts-node app.ts", "console": "CONSOLE=1 ts-node app.ts" },
 ...
 *   cd samples/nodejs/hireflow && npm install
 *
 * HOW TO RUN
 ...
 *
 *   npx ts-node app.ts              ← server mode (dashboard at /dashboard)
 *   CONSOLE=1 npx ts-node app.ts   ← console demo
 *
 * REST API
 *   POST /api/screen/start          → { candidate_id } → { session_id, agent_message }
 *   POST /api/screen/message        → { session_id, text } → { text, is_complete, scorecard? }
